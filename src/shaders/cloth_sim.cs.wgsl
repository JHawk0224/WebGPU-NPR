...
fn simulateCloth(@builtin(global_invocation_id) GlobalInvocationID: vec3<u32>) {
    ...
    let currentPosition = vertices.vertices[idx].position;
    ...
    vertices.vertices[idx].position = newPosition;
    velocities[idx] = velocity;

    // Constraints (pin top row)
    if (idx < uniforms.gridWidth) {
        vertices.vertices[idx].position = currentPosition; // Pin the top row
        velocities[idx] = vec3f(0.0, 0.0, 0.0);
    }
}
